...
//        new ArCodeRunnerDialogWrapper(e.getProject(), true).showAndGet();
        ...
        Boolean trainFromScratch = arCodePersistentStateComponent.getState().getTrainFromScratch();
        Boolean testFromScratch = arCodePersistentStateComponent.getState().getTestFromScratch();

        JFrame arCodeRecomFrame = null;
        try {
            arCodeRecomFrame = new ArCodeRecomFrameBuilder().createFrame(arcodeJarPath, framework, frameworkJarPath, frameworkPackage, trainProjectsPath, minerType, exclusionFilePath, testProjectsPath, trainProjectsPath, trainFromScratch, testFromScratch);
